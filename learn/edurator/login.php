...
switch($mode){ 
	...
	case 'forgot_pass':
		...
		if (isset($_POST['Send']))
		{
			...
			$sql = "SELECT id, username, name, email, power
						FROM pm_users 
						WHERE ";
			if (isset($email))
			{
				$sql .= " email LIKE '". str_replace("\'", "''", $email) ."'";
			}
			else
			{
				$sql .= " LOWER(username) = '". strtolower($username) ."'";
			}
			...
			else
			{
				...
				else
				{
					...
					require_once("include/class.phpmailer.php");
						...
						{
							@log_error(secure_sql('Error: Email template "email_forgot_password.txt" not found!'), 'User Login Page', 1);
							$mail = TRUE;
						}
						if($mail !== TRUE)
						{
