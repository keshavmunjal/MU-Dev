...
if ($id > 0)
{
	...
	if ( ! array_key_exists('type', $result))
	{
		...
		$article['link'] = art_make_link('article', array('id' => $article['id'], 'title' => $article['title']));
		...
		if ($article['allow_comments'] == 1)
		{
			...
			if ($comment_count > 0)
			{
				...
				// remove duplicate
				if ($config['comment_default_sort'] == 'score')
				{
		...
		$related_articles = get_related_article_list(array_pop(explode(',', $article['category'])), $article['title'], 4);
...
//	Some HTML 
$article_categories = art_html_list_categories($article['category'], $categories);
...
// Facebook Like
if ($config['show_fb_like'] == '1')
{
	$smarty->assign('facebook_like_href', urlencode($article['link']));
}
// Facebook image src
if (is_array($article) && $article['meta']['post_thumb_show'] != '')
{
	$facebook_image_src =  _ARTICLE_ATTACH_DIR . $article['meta']['post_thumb_show'];
