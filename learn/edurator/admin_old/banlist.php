	...
	case 'show':
		...
		else if($action == 'delete')
		{
			if(!empty($userid))
			{
				...
				if(!$result)
				{
					$info_msg = '<div class="alert alert-error">An error occurred!<br />MySQL Reported: '.mysql_error().'</div>';
				}
				else
				{
					$info_msg = '<div class="alert alert-success">The ban list was updated!</div>';
				}
			}
			else
			{
				$info_msg = '<div class="alert alert-error">"'.$userid.'" is not a valid user ID.</div>';
			}
			echo $info_msg;
		}
		...
		else if (isset($_POST['Submit']) && $action == 'ban')
		{
			...
			else
			{
				...
				{
					$info_msg = '<div class="alert alert-error">User not found</div>';
				}
				else
				{
					...
					{
						$info_msg = '<div class="alert alert-error">You can\'t do that.</div>';
					}
					else if ($info['power'] != U_ADMIN)
					{
						...
						if ( ! $result)
						{
							...
							$info_msg = '<div class="alert alert-success">The ban list was successfully updated!</div>';
					}
					else
						$info_msg = '<div class="alert alert-error">Administrator accounts cannot be banned!</div>';
				}
			}
			echo $info_msg;
		}
...
<a href="#banUser" role="button" class="btn btn-small btn-danger" data-toggle="modal">Ban user</a>
</div><!-- .btn-group -->
</div><!-- .qsFilter -->
</div>
...
<table cellpadding="0" cellspacing="0" width="100%" class="table table-striped table-bordered pm-tables tablesorter">
 ...
 <tbody>
  <?php echo $banlist; ?>
  <tr><td colspan="4" class="tableFooter"><div class="pagination pull-right"><?php echo $pagination; ?></div></td></tr>
 </tbody>
</table>
...
<button type="submit" name="Submit" value="Ban" class="btn btn-danger" />Ban user</button>
