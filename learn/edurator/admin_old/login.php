...
if (is_user_logged_in() && (is_admin() || is_moderator() || is_editor())) 
{
	...
	if( $redir === false || $redir == '/index.php')
	{ 	
...
if ($_POST['Login'] == 'Login')
{
	...
	if ($user != '' && (ctype_alnum($user) === TRUE) && $clearpass != '')
	{
		...
		if ($count == 0) 
		{
			$error = '<div class="alert alert-error">Your username and password don\'t match.</div>';
			@log_error('Failed attempt to log in to Admin Area. (Username used: <em>'.$user.'</em> / Password used: <em>'.htmlentities($clearpass, ENT_QUOTES).'</em> / IP: <em>'.$_SERVER['REMOTE_ADDR'].'</em>)', 'Admin login', 1);
		}
		else if ($count == 1) 
		{
			if ( ! in_array($row['power'], array(U_ADMIN, U_MODERATOR, U_EDITOR)))
			{
				$error = '<div class="alert alert-error">You do not have permission to access this area.</div>';
				@log_error('Failed attempt to log in to Admin Area. (Username used: <em>'.$user.'</em> / Password used: <em>'.htmlentities($clearpass, ENT_QUOTES).'</em> / IP: <em>'.$_SERVER['REMOTE_ADDR'].'</em>)', 'Admin login', 1);
			}
			else 
			{
	...
	{
		$error = '<div class="alert alert-error">Please enter your username and password.</div>';
	}
}
...
body{background:url('img/login-bg.png') repeat #d3d6d9;color:#626262;font-family:'Helvetica Neue',Helvetica,Arial Geneva,sans-serif;font-size:12px;margin:0;padding:0}
...
.login-form{margin:0 auto;padding:35px 30px;background-color:#FFF;width:300px;border:1px solid #becbd6;text-align:left;box-shadow:0 2px 2px #b5b7ba;-moz-box-shadow:0 2px 2px #b5b7ba;-webkit-box-shadow:0 2px 2px #b5b7ba;border-radius:5px;-moz-border-radius:5px;-webkit-border-radius:5px}
.login-input{display:block;margin-bottom:16px}
.forgot{font-size:11px;font-weight:bold;-webkit-font-smoothing:antialiased;color:#757677;text-shadow:0 1px 0 #FFF;margin:0 auto;margin-top:12px;text-align:center}
.forgot a,.forgot a:hover,.forgot a:visited{color:#45a4e1}
</style>
</head>
<body>
  ...
  <div class="container">
    ...
    <div class="row-fluid fade-in two">
      <div class="span12">
        <form action="login.php" method="post" name="login" onSubmit="return checkFields();" class="form-horizontal login-form">
          ...
          <fieldset>
            <div class="login-input">
              <input type="text" name="ausername" class="span12 input-login" placeholder="Username">
            </div>
            <div class="login-input">
        ...
        <div class="forgot">Forgot your password? <a href="<?php echo _URL."/login."._FEXT."?do=forgot_pass";?>" target="_blank">Click here</a>.</div>
