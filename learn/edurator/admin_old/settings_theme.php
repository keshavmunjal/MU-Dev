...
if ($_POST['action'] == 'delete-logo') // AJAX
{
	session_start();
	...
	$filename = array_pop(explode('/', $config['custom_logo_url']));
	...
	echo json_encode(array('success' => true,
							'msg' => '<div class="alert alert-info">The logo was deleted.</div>'
						  ));
...
else if ($_POST['submit'] == "Save")
{
	...
	$num_fields = array('new_videos', 'article_widget_limit', 'chart_days', 'top_videos', 'fav_limit', 'browse_page', 'comments_page', 'thumb_video_w', 'thumb_video_h', 'thumb_article_w', 'thumb_article_h', 'thumb_avatar_w', 'thumb_avatar_h', 'chart_days', 'show_stats', 'show_tags', 'tag_cloud_limit', 'search_suggest', 'show_fb_like', 'browse_articles');
...
<?php echo csrfguard_form('_admin_settings'); ?>
        ...
        <h2 class="h2-import pull-left">Layout Settings</h2>
            ...
            <li class="<?php echo ($_POST['settings_selected_tab'] == 't2') ? 'active' : '';?>"><a href="#t2" data-toggle="tab" class="tab-pane<?php echo ($_POST['settings_selected_tab'] == 't2') ? ' active' : '';?>">Customize Theme</a></li>
            <li class="<?php echo ($_POST['settings_selected_tab'] == 't3') ? 'active' : '';?>"><a data-toggle="tab" href="#t3" class="tab-pane<?php echo ($_POST['settings_selected_tab'] == 't3') ? ' active' : '';?>">Theme Store</a></li>
            </ul>
        </nav>
...
<table width="100%" border="0" cellspacing="1" cellpadding="0">
    ...
    $(document).ready(function(){
        $('#ButtonPlaceHolder').swfupload({
            ...
            button_width: "124",
            button_height: "20",
            button_text: '<span class="button">Upload logo</span>',
            button_text_style: '.button { text-align: center; font-size: 11px; font-weight: normal;font-family: "Lucida Grande", "Lucida Sans Unicode", "Lucida Sans", Geneva, Verdana, sans-serif;letter-spacing:0; padding:0;margin:0; }',
	...
	<table cellpadding="0" cellspacing="0" width="100%" class="table table-striped table-bordered pm-tables pm-tables-settings">
      ...
      <tr>
          ...
          <td>
            ...
            <span class="btn btn-small btn-logo-upload"><span id="ButtonPlaceHolder"></span></span>
      ...
      <tr>
          <td>Meta keywords</td>
          <td>
              <input name="homepage_keywords" type="text" size="45" value="<?php echo $config['homepage_keywords']; ?>" />
          </td>
      </tr>
      <tr>
          <td valign="top">Meta description</td>
          <td>
              <textarea name="homepage_description" rows="2" cols="55"><?php echo $config['homepage_description']; ?></textarea>
          </td>
      </tr>
	 	<tr>
	...
	<table cellpadding="0" cellspacing="0" width="100%" class="table table-striped table-bordered pm-tables pm-tables-settings">
	 ...
	 <tr>
        ...
        <?php if ( $config['mod_article'] != 1 ) { ?><span class="label label-warning">The Article Module is currently disabled.</span><?php } ?>
        </td>
	 </tr>
	 <tr>
		<td>User Avatar</td>
    ...
    <h2 class="sub-head-settings">Other</h2>
	<table cellpadding="0" cellspacing="0" width="100%" class="table table-striped table-bordered pm-tables pm-tables-settings">
    <tr>
        <td width="20%">Show Facebook Like button (under the video player)</td>
        <td>
            <label><input name="show_fb_like" type="radio" value="1" <?php echo ($config['show_fb_like']==1) ? 'checked="checked"' : "";?> /> Yes</label>  
            <label><input name="show_fb_like" type="radio" value="0" <?php echo ($config['show_fb_like']==0) ? 'checked="checked"' : "";?> /> No</label>
            <a href="#" rel="popover" data-placement="right" data-trigger="hover" data-content="If enabled, a FaceBook Like button will appear under all videos."><i class="icon-info-sign"></i></a>
        </td>
    </tr>
    </table>
	</div>
    ...
    <div class="pm-themes">
		...
		if (is_array($data) && count($data) > 0) : 
			...
			<?php else : ?>
				<div class="alert alert-warning">No themes available at the moment. <strong><a href="http://www.phpsugar.com/phpmelody_templates.html?utm_source=install_footer" target="_blank">Click here</a></strong> to visit the Theme Store.</div>
			<?php endif; ?>
		<?php else : ?>
			<div class="alert alert-warning">Sorry, couldn't retrieve data from the Theme Store. <strong><a href="http://www.phpsugar.com/phpmelody_templates.html?utm_source=install_footer" target="_blank">Click here</a></strong> to visit the Theme Store.</div>
		<?php endif;?>
    </div><!--.pm-themes-->
