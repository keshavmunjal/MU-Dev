...
jQuery(document).ready(function($){
	...
	Gritter = {
		...
		_tpl_item: '<div id="gritter-item-[[number]]" class="gritter-item-wrapper" style="display:none"><div class="gritter-top"></div><div class="gritter-item"><div class="nottype[[bgcolor]]">[[image]]</div><div class="[[class_name]]"><span class="gritter-title">[[username]]</span><p>[[text]]</p></div><div style="clear:both"></div></div><div class="gritter-bottom"></div></div>',
	    _tpl_wrap: '<div id="gritter-notice-wrapper"></div>',
	    
	    // Add a notification to the screen
	    add: function(user, text, image, bgcolor, sticky, time_alive){
			...
			var image_str = (image != '') ? '<div class="gritter-ico"><img src="' + image + '" /></div>' : '';
			var class_name = (image != '') ? 'gritter-with-image' : 'gritter-without-image';
			
	        tmp = this.str_replace(
	            ['[[username]]', '[[text]]', '[[image]]', '[[bgcolor]]', '[[number]]', '[[class_name]]'],
	            [user, text, image_str, bgcolor, this._item_count, class_name], tmp
	        );
	    ...
	    remove: function(e){
			...
			$.cookie('showNotice', 'off', { expires: 2 }); // set cookie with an expiration date seven days in the future
	    },
