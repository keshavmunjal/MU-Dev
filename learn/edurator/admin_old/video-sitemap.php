...
function vsitemap_item($item = array(), $args = array()) 
{
	...
	$link = makevideolink($item['uniq_id'], $item['video_title']);
	...
	{
		$ext = strtolower(array_pop(explode('.', $item['url_flv'])));
...
if ($_GET['do'] == 'map')
{
	...
	if ( ! in_array($file, array('video-sitemap-1.xml', 'video-sitemap-2.xml')))
	{
		$ajax_state = 'error';
			...
			success: function(data){
						switch (data['state'])
						{
							...
							case 'error':
								...
								$('#build-map-button').attr('disabled', '');
							break;
						}
					},
			dataType: 'json'
...
<table cellpadding="0" cellspacing="0" width="100%" class="table table-striped table-bordered pm-tables pm-tables-settings">
      ...
      <tr class="table_row1">
		...
		<a href="#" rel="popover" data-placement="right" data-trigger="hover" data-content="If your database is large you should split your videos into two sitemap files."><i class="icon-info-sign"></i></a>
		</td>
     </tr>
      <tr class="table_row1">
...
<button type="submit" name="submit" data-loading-text="Generating..." class="btn btn-small btn-success" id="build-map-button" />Generate sitemap</button>
