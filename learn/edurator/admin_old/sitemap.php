...
{	
	...
	if (mysql_num_rows($results) > 0)
	{
		...
		while ($row = mysql_fetch_array($results))
		{
			...
			$smap .= "<url>\r\n";
			$smap .= "<loc>".clean_feed( makevideolink($row['uniq_id'], $row['video_title']) )."</loc>\r\n";
			$smap .= "<lastmod>".date('Y-m-d')."</lastmod>\r\n";
			$smap .= "<changefreq>weekly</changefreq>\r\n";
			$smap .= "</url>\r\n";
		}
		...
		if ($config['mod_article'] == 1)
		{
			...
			if (mysql_num_rows($result) > 0)
			{
				...
				{
					$date = date('Y-m-d', $row['date']);
					$loc = art_make_link('article', array('id' => $row['id'], 'title' => $row['title']));
	...
	{
		log_error(secure_sql('MySQL Error: '. mysql_error()), 'Generate Sitemap', '1') ;
