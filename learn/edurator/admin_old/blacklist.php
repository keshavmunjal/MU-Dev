...
if($_POST['Submit'] == "Save"){
	...
	$fp = fopen($file, "w");
	if(!$fp) { 
		echo '<div class="alert">Sorry, file <strong>'.$file.'</strong> cannot be opened. Check if <strong>'.$file.'</strong> was uploaded and if it\'s writtable (CHMOD 0777)</div>';
		include('footer.php');
		exit();
	}
	$line = '';
	...
	fclose($fp);
	$info_msg = '<div class="alert alert-success">The list was updated successfully.</div>';
}
else{
	$fp = @fopen($file, "r");
	if(!$fp) { 
	...
	fclose($fp);
if(empty($content)) $info_msg = '<div class="alert alert-info">The list is currently empty. Add one word per line (i.e. a list) without any punctuation marks.</div>';
}
...
function read_censored_words($filename) {
		...
		$content = '';
		if(!$fp) { 
			return 'Could not open file '.$file.'. Make sure the file has been uploaded and it is writtable (CHMOD 0777)';
			//include('footer.php');
			//exit;
		}
		while( !feof($fp)){
...
?>
	<div class="alert alert-info">Keep your site clean and your SEO rankings in good standing by filtering in any obscene or unwanted words from video and article comments.</div>
