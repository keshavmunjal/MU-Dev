	...
	if($video != 0) 
	{
		...
		if($res['total'] == 0)
		{
			...
			if($res['total'] < _FAV_LIMIT)
			{
				...
				echo json_encode(array('success' => true, 'btn_label' => $lang["remove_from_fav"], 'msg' => '', 
									   'html' => '<button class="btn btn-small border-radius0 active" id="fav_save_button" type="button" data-loading-text="Saving...">'.$lang["remove_from_fav"].'</button>'));
				exit();
			}
		}
		else // remove from favorites
		{
			...
			echo json_encode(array('success' => true, 'btn_label' => 'Add to favorites', 'msg' => '', 
								   'html' => '<button class="btn btn-small border-radius0" id="fav_save_button" type="button" data-loading-text="Saving...">'.$lang["add_to_fav"].'</button>'));
			exit();
		}
	}
}
