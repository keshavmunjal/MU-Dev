...
switch ($page)
{
	...
	case 'video':
		...
		switch ($action)
		{
			...
			case 'getplayer': // called after a Pre-roll ad has finished running
				...
				if ($player_page == '' || ! in_array($player_page, array('index', 'detail', 'favorites')))
				{
					$player_page = 'detail';
				}
				...
				if ($ad_id > 0 && $config['total_preroll_ads'] > 0)
				{
					@mysql_query("UPDATE pm_preroll_ads SET impressions=impressions+1 WHERE id = '$ad_id'");
				}
	...
	case 'users':
		...
		if (is_admin() || (is_moderator() && mod_can('manage_users')))
		{
			...
			if ( !  $result)
			{
				log_error(secure_sql('MySQL Error: '. mysql_error() . '<br>File: '. __FILE__ .' on line '. __LINE__), 'User Management', '1');
		...
		switch ($action)
		{
			...
			case 'report':
				...
				if ($action == 'share')
				{
					...
					{
						@log_error(secure_sql('Error: Email template "email_send_to_friend.txt" not found!'), 'Share Video', 1);
						$mail = TRUE;
					}
					if($mail !== TRUE)
					{
