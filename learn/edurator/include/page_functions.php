...
{
	$page = array();
	$int_values = array('id', 'status', 'author', 'date', 'views');
	$page_id = (int) $page_id;
	...
	$page['meta_keywords'] = $meta_data['meta_keywords'];
	$page['meta_description'] = $meta_data['meta_description'];
...
function insert_new_page($postarr)
{
	...
	$sql = "INSERT INTO pm_pages 
					(title, content, author, date, status, page_name, views) 
			VALUES  ('". $title ."', 
					 '". $content ."', 
					 '". $author ."', 
					 '". time() ."', 
	...
	update_meta($last_id, IS_PAGE, 'meta_keywords', $postarr['meta_keywords']);
	update_meta($last_id, IS_PAGE, 'meta_description', $postarr['meta_description']);
...
function update_page($id, $postarr)
{
	...
	update_meta($id, IS_PAGE, 'meta_keywords', $postarr['meta_keywords']);
	update_meta($id, IS_PAGE, 'meta_description', $postarr['meta_description']);
...
function list_pages($search_term, $search_type = 'title', $from = 0, $to = 20, $filter = '', $filter_value = '') 
{
	...
	if (mysql_num_rows($result) > 0)
	{
		while ($row = mysql_fetch_assoc($result))
		{
			...
			$pages[$i]['meta_keywords'] = unspecialchars($meta_data['meta_keywords']);
			$pages[$i]['meta_description'] = unspecialchars($meta_data['meta_description']);
