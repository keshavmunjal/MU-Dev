...
function list_sub_child($parent) 
{ 
	if (empty($parent))
	{
		return '';
...
function makevideolink($uniq_id, $video_title = '')
{
	...
	if (_SEOMOD == 1) 
	{
		...
		$return = _URL .'/'. $video_title_clean .'-video-'. $uniq_id .'.html';
	} 
	else 
	{
...
function get_featured_video_list($orderby = '', $sort = '', $start = 0, $limit = 5, $category_id = 0, $video_ids = array(), $uniq_ids = array()) 
{
	//echo "orderby".$orderby."short".$sort."start".$start."limit".$limit."category id:".$category_id;exit;
	$sql = "SELECT * FROM pm_videos 
			 WHERE added <= '". time() ."'";
...
function get_category_name($categoryId){
	...
	if(!empty($catId)){
		$sql_statement = "SELECT * FROM pm_categories WHERE id =".$catId;
	}else{
		$sql_statement = "SELECT * FROM pm_categories WHERE id IN ('".$categoryId."')";
	}
...
function get_sub_category(){
	...
	$sql_statement = "SELECT parent_id, name FROM pm_categories";
	$qry = mysql_query($sql_statement);
	$catArray[] = '';
	while($sqlQuery = mysql_fetch_array($qry)){
		$catArray[] = $sqlQuery;
	}
	return $catArray; 
...
function TotalVedioForExplore($sub_cate_id)
	{
		$query = mysql_query("SELECT sum(total_videos) as tv FROM pm_categories WHERE parent_id='".$sub_cate_id."'");
		while($data=mysql_fetch_array($query))
			{
				//$abc=$data['total_videos'];
				$abc = $data[0];
			}
			return $abc;
	}
function ForExploreUniv()
	{
		$query = mysql_query("select * from pm_videos where category in(49) order by rand() limit 3");
		
		$catArray[] = '';
		while($sqlQuery = mysql_fetch_array($query)){
		$catArray[] = $sqlQuery;
			}
	return $catArray; 
...
function get_Toatal_video_list($catId)
{
	$query = mysql_query("SELECT total_videos FROM `pm_categories` where parent_id = '".$catId."' or id ='".$catId."'");
		
		$total=0;
		while($sqlQuery = mysql_fetch_array($query)){
			$total=$total+$sqlQuery['total_videos'];
			}
	return $total;
}

function getfeaturedvideobycat($catId)
{
	$query = mysql_query("SELECT *  FROM `pm_videos` WHERE `category` LIKE '".$catId."' AND featured =1 ORDER BY RAND() LIMIT 1");
