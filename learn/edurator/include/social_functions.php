...
function get_following_list($follower_id = -1, $start = 0, $limit = 30)
{
	...
	while ($row = mysql_fetch_assoc($result))
	{
		...
		$leaders[$row['user_id']]['last_seen'] = ucwords(time_since($row['last_signin'], true)).' '.$lang['ago'];
		...
		if ($row['object_id'] != 0)
		{
			...
			switch ($row['object_type'])
			{
				...
				case ACT_OBJ_VIDEO:
					...
					$meta['object']['video_href'] = makevideolink($meta['object']['uniq_id'], $meta['object']['video_title']);
				...
				case ACT_OBJ_ARTICLE:
					...
					$meta['object']['link'] = art_make_link('article', array('id' => $meta['object']['id'], 'title' => $meta['object']['title']));
		...
		if ($row['target_id'] != 0)
		{
			switch ($row['target_type'])
			{
				...
				case ACT_OBJ_VIDEO:
					...
					$meta['target']['video_href'] = makevideolink($meta['target']['uniq_id'], $meta['target']['video_title']);
				...
				case ACT_OBJ_ARTICLE:
					...
					$meta['target']['link'] = art_make_link('article', array('id' => $meta['target']['id'], 'title' => $meta['target']['title']));
