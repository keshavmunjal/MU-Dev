...
$twitter_status .= ' '. makevideolink($video['uniq_id'], $video['video_title']);
$twitter_status = urlencode($twitter_status);

// define meta tags
$meta_title = $video['video_title'];
$meta_description = generate_excerpt(str_replace('"', '&quot;', $video['excerpt']), 70) .'...';
...
$same_category_id = array_pop(explode(',', $video['category']));
$related_video_list = get_related_video_list($same_category_id, $video['video_title'], 10);
$top_rated_video_list = get_top_rated_video_list($same_category_id, 10);
...
$video_link = urldecode(makevideolink($video['uniq_id'], $video['video_title']));
...
$smarty->assign('show_fb_like', $config['show_fb_like']);

$smarty->assign('embedcode_to_share', generate_embed_code($video['uniq_id'], $video));
$smarty->assign('embedcode', generate_embed_code($video['uniq_id'], $video, false));
