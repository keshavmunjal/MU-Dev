...
if ($logged_in)
{
	...
	{ 
		...
		if (in_array($_FILES['imagefile']['type'], $allowed_type) && in_array($file_ext, $whitelist))
		{
			...
			$copy = @copy($_FILES['imagefile']['tmp_name'], "$tdir" . $new_name); 
			if ($copy === TRUE)
			{
				$resize = resize_then_crop("$tdir". $new_name, "$tdir". $avatar_name, THUMB_W_AVATAR, THUMB_H_AVATAR, "255", "255", "255", $allowed_type);
				if($resize != false)
				{
					if ($userdata['avatar'] != 'default.gif' && $userdata['avatar'] != 'no_avatar.gif')
					{ 
						@unlink($tdir. $userdata['avatar']);
					}
